...
// Six entries so the README section limit of five is exercised.
...
func TestRenderReadme(t *testing.T) {
	...
	got := renderReadme(now, testRepos, testPullRequests, testIssues, testDiscussions)
	assertGolden(t, "readme.golden.md", got)
}
...
func TestRenderSeeAll(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			title:  "All pull requests I created",
			items:  testPullRequests,
			golden: "pull-requests.golden.md",
		},
		{
			name:   "issues without state and with unsorted input",
			title:  "All issues I participated in",
			items:  testIssues,
			golden: "issues.golden.md",
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			assertGolden(t, tt.golden, renderSeeAll(tt.title, tt.items))
		})
	}
}
